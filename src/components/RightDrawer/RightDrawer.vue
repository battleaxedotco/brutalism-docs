<template>
  <div class="right-drawer" v-if="isMounted">
    <div
      ...
      :class="[
        'right-drawer-item',
        checkItemStatus(anchor) ? 'active' : 'idle',
      ]"
      :style=""
    >
      <span
        v-scroll-to="{ el: `#${anchor.id}`, offset: 6 }"
        class="right-drawer-item-label"
        >{{ anchor.name }}</span
      >
    </div>
  </div>
...
export default {
    ...
    realPos(val) {
      try {
        ...
        if (!targetRange && val < this.anchors[0].range[0]) {
          ...
          !targetRange &&
          val > this.anchors[this.anchors.length - 1].range[0]
        ) {
  ...
  methods: {
    ...
    calculateScrollPercentage() {
      try {
        if (
          this.activeAnchor.index !== this.anchors.length - 1 &&
          this.realPos > this.anchors[0].range[0]
        ) {
          this.scrollPercentage = Math.floor(
            ((this.realPos - this.activeAnchor.range[0]) /
              (this.activeAnchor.range[1] - this.activeAnchor.range[0])) *
              100
          );
          this.miniScrollTop =
            this.activeAnchor.index * 40 + (40 * this.scrollPercentage) / 100;
        } else if (
          this.realPos > this.anchors[this.anchors.length - 1].range[0]
        ) {
